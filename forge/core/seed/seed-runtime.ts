 ...
 * - waitForL1Idle polling between batches
 * - waitForAllIdle polling before shutdown (L1 + L2 + L3)
...
/**
 * Poll pipeline queue status until L1, L2, and L3 are all idle.
 * This ensures L2 (scene extraction) and L3 (persona generation) have
 * completed before the pipeline is destroyed.
 */
async function waitForAllIdle(
  ...
  const pollInterval = opts.pollIntervalMs ?? 1_000;
  const stableRounds = opts.stableRounds ?? 3;
  const maxWait = opts.maxWaitMs ?? 1_800_000; // 30 min default
  ...
  while (true) {
    const elapsed = Date.now() - startTime;
    if (elapsed > maxWait) {
      logger.warn(`${TAG} [waitAll] Max wait time reached (${(maxWait / 1000).toFixed(0)}s), proceeding`);
      break;
    }

    const queues = scheduler.getQueueSizes();

    if (queues.l1Idle && queues.l2Idle && queues.l3Idle) {
      consecutiveIdle++;
      if (consecutiveIdle >= stableRounds) {
        logger.debug?.(`${TAG} [waitAll] All queues stable for ${stableRounds} consecutive polls`);
        return;
      }
    } else {
      consecutiveIdle = 0;
      logger.debug?.(
        `${TAG} [waitAll] Waiting: l1Idle=${queues.l1Idle}, l2Idle=${queues.l2Idle}, l3Idle=${queues.l3Idle}`,
      );
    }
...
/**
 * Execute the seed pipeline: feed normalized input through L0 → L1 → L2 → L3.
 *
 * L2/L3 runners are wired and their completion is awaited before the pipeline
 * is destroyed, so the final output includes the latest scenes and persona.
 *
...
export async function executeSeed(
  ...
  try {
    ...
    // Final wait for all sessions (L1 + L2 + L3)
    if (!interrupted) {
      logger.info(`${TAG} Final L1+L2+L3 idle wait for all sessions...`);
      await waitForAllIdle(
        pipeline.scheduler,
        logger,
        { pollIntervalMs: 1_000, stableRounds: 3, maxWaitMs: 1_800_000 },
      );
    }
  } finally {
